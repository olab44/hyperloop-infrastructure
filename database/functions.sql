...
CREATE OR REPLACE FUNCTION CalculateRouteLength(route_id INT)
RETURNS INT
DETERMINISTIC
BEGIN
...
CREATE OR REPLACE FUNCTION CalculateRouteTime(route_id INT)
RETURNS DECIMAL(10, 2)
DETERMINISTIC
BEGIN
    DECLARE total_time DECIMAL(10, 2) DEFAULT 0.0;

    SELECT SUM(s.LENGTH / s.MAX_SPEED) INTO total_length, total_time
    FROM
        ROUTE_STRETCH rs
        JOIN STRETCH s ON rs.STRETCH_ID = s.STRETCH_ID
    WHERE
